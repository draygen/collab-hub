#!/usr/bin/env python3
"""
Collab Server — Real-time collaboration hub for Claude agents.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _sse_loop(self, agent: str):
        ...
        log({"event": "sse_connect", "agent": agent})
        ...
        try:
            ...
            with sub_lock:
                try:
                    ...
                    pass
            log({"event": "sse_disconnect", "agent": agent})
    ...
    def do_POST(self):
        ...
        if path == "/send":
            ...
            self.send_json({"ok": True, "task": task} if task else {"error": "not found"}, 200 if task else 404)
